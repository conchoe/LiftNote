...
export default function FollowersScreen() {
  ...
  const load = useCallback(async () => {
    ...
    const ids = [...new Set(links.map((r) => r.user_id as string))];
  ...
  if (!authEnabled || !session) {
    return (
      <SafeAreaView style={styles.safe} edges={["bottom", "left", "right"]}>
        <Text style={styles.muted}>Sign in to see followers.</Text>
      </SafeAreaView>
    );
  }
    ...
    <SafeAreaView style={styles.safe} edges={["bottom", "left", "right"]}>
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={<Text style={styles.muted}>No followers yet.</Text>}
          renderItem={({ item }) => (
